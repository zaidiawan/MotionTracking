...
GrowRegion::GrowRegion(IplImage* srcImage){
    ...
    //tempImage = cvCreateImage(cvSize(640,480), IPL_DEPTH_8U,1);
    //cvSet(tempImage, cvScalar(0));
...
void GrowRegion::run(IplImage* srcMatInput, Point SeedPointInput,int distance,Point& stopPointInput,vector<Point>& borderPoints,Mat& returnedBorderMat){
    ...
    /*cvCircle(cloneLookUpDepthImageDifferently,cvPoint(SeedPointInput.x,SeedPointInput.y),5,CV_RGB(255,255,255),1);
    cvShowImage("edge region-grow",cloneLookUpDepthImageDifferently);*/

    /*cv::cvarrToMat(cloneLookUpDepthImageDifferently).copyTo(returnedBorderMat);
    dilate(returnedBorderMat,returnedBorderMat,5);*/
        /*cvCircle(borderImage,cvPoint(stopPointInput.x,stopPointInput.y),5,CV_RGB(255,255,255),1);
        cvShowImage("borderImage",borderImage);*/
...
void GrowRegion::getNewSeedPoints(vector<Point> SeedPoints){
    ...
    if(SeedPoints.size() != 0){
        ...
        for(int j = 0; j < maxLimit1; j++){
            ...
            int maxLimit2 = neighbourPoints.size();
