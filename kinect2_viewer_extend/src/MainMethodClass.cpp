...
void MainMethodClass::createTrackbars(){
    ...
    /*cv::createTrackbar( "Y_MIN", "Trackbars", &Y_MIN, 256, 0 );
    cv::createTrackbar( "Y_MAX", "Trackbars", &Y_MAX, 256, 0 );
    cv::createTrackbar( "Cr_MIN", "Trackbars", &Cr_MIN, 256, 0 );
    cv::createTrackbar( "Cr_MAX", "Trackbars", &Cr_MAX, 256, 0 );
    cv::createTrackbar( "Cb_MIN", "Trackbars", &Cb_MIN, 256, 0 );
    cv::createTrackbar( "Cb_MAX", "Trackbars", &Cb_MAX, 256, 0 );*/


    //cv::createTrackbar( "DisplayHist", "Trackbars", &displayHistogramFlag, 1, 0 );
    //createTrackbar("erodeAmt","Trackbars",&erodeAmt,16);
    //createTrackbar("dilateAmt","Trackbars",&dilateAmt,16);

    //createTrackbar("medianBlurCnt","Trackbars",&medianBlurCnt,16);
    //
    ////for backGroundSubtraction
    //createTrackbar("backGrndSub","Trackbars",&backGroundSubtractionFlag,1);
    //
    //createTrackbar("Kalman","Trackbars",&flagForKalman,1);
 //
    ////createTrackbar("writeToFileFlag","Trackbars",&writeToFileFlag,1);
    ////createTrackbar("flagForWaterShade","Trackbars",&flagForWaterShade,1);
    //createTrackbar("handDetectionFlag","Trackbars",&handDetectionFlag,1);
...
//	 for(int y=1,y1 = 0; y<height; y++,y1++){
//		for(int x=1,x1=0; x<width; x++,x1++){
//
//			unsigned short value = ((unsigned short *)&(srcMat->imageData[srcMat->widthStep * (y*4)]))[x*4];
//
//			if(value == 0){
//				vector<unsigned short> pointsValues;
//				unsigned short value1;
//			 //x-1,y-1
//			 value1 = ((unsigned short *)&(srcMat->imageData[srcMat->widthStep * (y*4 -1 )]))[x*4 - 1];
//
//			 if(value1 != 0 && value1 >= 500){
//				 pointsValues.push_back(value1);
//			 }
//
//			 //x,y-1
//			 value1 = ((unsigned short *)&(srcMat->imageData[srcMat->widthStep * (y*4 -1 )]))[x*4];
//
//			 if(value1 != 0 && value1 >= 500){
//				 pointsValues.push_back(value1);
//			 }
...
//
//			 //x-1,y
//			 value1 = ((unsigned short *)&(srcMat->imageData[srcMat->widthStep * (y*4)]))[x*4-1];
//
//			 if(value1 != 0 && value1 >= 500){
//				 pointsValues.push_back(value1);
//			 }
...
//
//
//			 //find the lowest point value in a vector...
//			 unsigned short min = ((unsigned short*)(srcMat->imageData + srcMat->widthStep*(y*4)))[x*4];
//			 for(int vc = 1 ; vc < pointsValues.size(); vc++){
//
//				 min = pointsValues[0];
//				 if(pointsValues[vc] < min){
//					min = pointsValues[vc];
//				 }
//			 }
//			 ((unsigned short*)(returnedIplImage->imageData +
//					returnedIplImage->widthStep*y1))[x1] = min;
//			 ((unsigned short*)(srcMat->imageData + srcMat->widthStep*(y*4)))[x*4] = min;
//			} else {
//				((unsigned short*)(returnedIplImage->imageData +
    ...
    for(int y=1; y<height; y++){
        for(int x=1; x<width; x++){
             ...
             //if(flickering == 1){
                ...
                // if(strength < 25){ //the strength of the stimulus must be greater than a certain point, else we do not want to allow the change
                ...
                //
                //		if(value > preValue){  // use the previous frames value.  Change it by +1 - slow enough to not be noticable flicker
                //			value = preValue + 1;
                //		}else{
                //			value = preValue - 1;
                //		}
             ...
             //else {
             //
                // //if pixel value is zero, ie shadow region..
                // if(x != 0)
                // ((unsigned short*)(srcMat->imageData + srcMat->widthStep*y))[x] = ((unsigned short*)(srcMat->imageData + srcMat->widthStep*y))[x-1];
...
//    erode(edeMat,edeMat,2);
//    dilate(edeMat,edeMat,2);
    //if(flickering == 1){
        //copyIplImageToOtherIplImage(srcMat,prevdeflickerIplImage);
    //}
    //const float scaleFactor = 0.05f;
    //Mat show1 = cvarrToMat(tempImageEdgeImage);
    //imshow("edeMat",edeMat);
    cvShowImage("edge image",tempImageEdgeImage);
...
void MainMethodClass::initializeVariables(cv::Mat& colorImage,cv::Mat& depthImage){
    ...
    pyrDown(colorMatPyrDown,colorMatPyrDown);
        ...
        //try to do and operation between two Depth Images, current and previous frames
        //depthImage = deflicker(depthImage,20);  // this is the function call

    //if(flagForFirstTime == true){
    //	//make it false..
    //	flagForFirstTime = false;
    //	depthImage.copyTo(depthMatOrgSizePreviousFrame);
    //} else {
    //	Mat tempCurrentFrameMat;
    //	depthImage.copyTo(tempCurrentFrameMat);
    //	//do and operation between current frame and next frame.
    //	depthImage = depthImage & depthMatOrgSizePreviousFrame;
    //	//Re-save current frame for next frame
    //	tempCurrentFrameMat.copyTo(depthMatOrgSizePreviousFrame);
    //}
...
//    Mat tempMat;
//    pyrDown(depthImage,tempMat);
//    pyrDown(tempMat,depthMatPyrDown);

//    IplImage copy1;
//    copy1 = depthMatPyrDown;
//    depthIplImagePyrDown = &copy1;
    ...
    /*if(depthDistanceLeft >= 500 || depthDistanceRight >= 500){
        if(depthDistanceLeft > depthDistanceRight){
            removeBackGroundBasedOnDepthAndCreateEdgeImage(depthIplImagePyrDown, depthDistanceLeft+100,depthIplBackGrndSubMask,edgeIplDepthImage);
        } else {
            removeBackGroundBasedOnDepthAndCreateEdgeImage(depthIplImagePyrDown, depthDistanceRight+100,depthIplBackGrndSubMask,edgeIplDepthImage);
        }
    } else {
...
void MainMethodClass::skinThresholding(){
    cvtColor(colorMatPyrDown, hsv, CV_BGR2HSV);
            ...
            //tries to implement motion detection with skin mask but its not working well
            /*
            imshow("HSV Skin Mask before",mask1);
            //subtract(mask1 , mask1PreviousFrame,mask1);
            Mat tempMask1;
            //mask1.copyTo(tempMask1);
            //cvtColor(tempMask1,tempMask1,CV_GRAY2BGR);
            //writeMatToFile(tempMask1,"mask1","mask1");
            tempMask1 = Mat::zeros(mask1.size(),mask1.type());
            int maxRows = mask1.rows;
            int maxCols = mask1.cols;
            int val1,val2,diff;
            for(int i = 0; i< maxRows;i++){
                for(int j = 0;j< maxCols;j++){
                    val1 = mask1.at<uchar>(i,j);
                    val2 = mask1PreviousFrame.at<uchar>(i,j);
            ...
            imshow("HSV Skin Mask After",mask1 );
            */
            ...
            /*intemediate1 =  clock() - intemediate;
                    cout<< "intemediate0 time : "<<(double)intemediate1/((double)CLOCKS_PER_SEC)<<endl;  */
            int ch[] = {0, 0};
...
void MainMethodClass::run(cv::Mat& colorImage,cv::Mat& depthImage,
         ...
         skinThresholding();
          ...
          initializeTrackingWindowRight();
            ...
            if(selectionLeft.height > 0 && selectionLeft.width > 0 &&
                ...
                //trackLeftHand(colorMatPyrDown,depthImage,depthIplImage,originalColorImage,originalDepthImage,openCVKinect,edgeDepthImage);
            ...
            if(selectionRight.height > 0 && selectionRight.width > 0 &&
                ...
                //trackRightHand(colorMatPyrDown,depthImage,depthIplImage,originalColorImage,originalDepthImage,openCVKinect);
                trackRightHand(rightHandCoordinates);

            }

            /*intemediate2 =  clock() - intemediate;
                    cout<< "intemediate2 time : "<<(double)intemediate2/((double)CLOCKS_PER_SEC)<<endl; 	 */
...
//            CV_FONT_HERSHEY_SIMPLEX|CV_FONT_ITALIC,2,Scalar(0,255,0),3,3);
        ...
        //writeMatToFile(mask1PreviousFrame,"mask1PreviousFrame","mask1PreviousFrame");
}
void MainMethodClass::trackLeftHand(Point3i& handCoordinates){
    ...
    //cout<<"Watch 0 : TW x & y : "<< trackWindowLeft.tl().x << " , "<< trackWindowLeft.tl().y <<endl;


    //cout<<"Watch 1 : calculateBackProjForLeft After "<<endl;
    //imshow("backprojLeft before",backprojLeft);
    ...
    if(trackWindowLeft.height > 0 && trackWindowLeft.width > 0 &&
     ...
     //cout<<"Watch 2 : CamShift After "<<endl;
...
//                        float x,y,z;
                        ...
                        if(flagForGesture || gesture == 1){
                            ...
                            int maxLimit1 = trajectory.size()-1;
                            ...
                            if(trajectory.size() != 0){
                                ...
                                Point3i point;
                                point = trajectory[0];
                                int maxLimit2 = trajectory.size();
                                for(int cn = 0; cn < maxLimit2; cn++){
                        ...
                        //for (int i = noOfTrajectoryPoints; i < trajectory.size()-1; i++)
                        /*int index = trajectory.size();
                        if(index > 1){
                            line(colorMatOriginalSize, Point(trajectory[index-1].x*4,trajectory[index-1].y*4),Point(point.x*4,point.y*4),
                                Scalar(0,0,255), 3, 8, 0);
                        }*/
                        //noOfTrajectoryPoints++;
                    //}
...
void MainMethodClass::findTheTopPointOfThePalm(vector<Point>&sortedPoints,Point& topPointPalm,Point& leftPointPalm,Point& rightPointPalm,String flag){
    ...
    if(flag == "top"){
        ...
        int maxLimit1 = sortedPoints.size();
        for(int i = 0;i < maxLimit1;i++){
...
//    Point startPointTemp = startPoint;
    ...
    if(trackWindowLeft.height >= trackWindowLeft.width){
        ...
        //for(int cn = 0; cn < srcVecPoints.size(); cn++){
        //
        //	//find stop point in srcVec..
        //	if(srcVecPoints[cn].y <=   startPoint.y ){
        //		sortVec1.push_back(srcVecPoints[cn]);

        //	} else if(srcVecPoints[cn].y >   startPoint.y ){
        //		sortVec2.push_back(srcVecPoints[cn]);
        //	}
        //}
        ...
        int maxLimit1 = tempSortVect.size();
        for(int ix = 0 ; ix < maxLimit1 ; ix++){
        ...
        //
        //	//find stop point in srcVec..
        //	if(srcVecPoints[cn].x <=   startPoint.x ){
        //		sortVec1.push_back(srcVecPoints[cn]);
    ...
    /*circle(show,leftPointPalm,2,Scalar(255,255,0),1);
    putText(show,"LP",Point(leftPointPalm.x-10,leftPointPalm.y-10),CV_FONT_HERSHEY_SIMPLEX|CV_FONT_ITALIC,0.5,Scalar(255,255,0),0.1,1);

    circle(show,rightPointPalm,2,Scalar(255,255,0),1);
    putText(show,"HP",Point(rightPointPalm.x-10,rightPointPalm.y-10),CV_FONT_HERSHEY_SIMPLEX|CV_FONT_ITALIC,0.5,Scalar(255,255,0),0.1,1);*/
        ...
        int i = findBiggestContour(contours);
                    ...
                    if(cArea > 700 ) // likely the hand
                    {
                        ...
                        if(bRect.width < 180 && bRect.height < 220){
                            ...
                            int maxLimit1 = convexDefects.size();
                            for (int zz = 0; zz < maxLimit1; zz++)
                                {
                            ...
                            int maxLimit2 = hull.size();
                            for(int k = 0; k < maxLimit2; k++)
                            {
...
void MainMethodClass::displayHistogram(Mat& srcHistMat,String hand){
    ...
    for( int i = 0; i < hsize; i++ )
        ...
        /*
        //for report, hist with all colors
        for( int i = 0; i < hsize; i++ )
            {
                int val = saturate_cast<int>((i+1)*15);
                rectangle( histimg, Point(i*binW,histimg.rows+2),
                    Point((i+1)*binW,histimg.rows - val),
                        Scalar(buf.at<Vec3b>(i)), -1, 8 );
            }*/
        if(hand == "left"){
...
void MainMethodClass::averageDepthImageValue(Mat srcMat,Rect roi, float& avgDepthValue){
    ...
    // just for cross check purpose..
    /*imwrite("DepthImages/tempDepthWindowMat.jpg",tempDepthWindowMat);
    imwrite("DepthImages/srcMat.jpg",srcMat);

    FileStorage fs("DepthImages/tempDepthWindowMat.txt", FileStorage::WRITE);
    fs << "depthMat" << tempDepthWindowMat;*/
